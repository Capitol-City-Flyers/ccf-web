...
import {DateTime, DateTimeUnit, DurationLike, DurationUnit, Interval, Zone} from "luxon";
...
export class DateCalc {
    private readonly dateTimeOpts: { zone: Zone };
    private readonly firstDayOfWeekAdjustment: DurationLike;

    private constructor(public readonly zone: Zone) {
        this.dateTimeOpts = freeze({zone});
    ...
    nowUTC() {
        return DateTime.local({zone: "UTC"});
    }
    ...
    remainderOf(count: number, unit: DurationUnit & DateTimeUnit, reference?: DateTime): Interval {
        ...
        const start = reference?.setZone(this.zone) || this.now();
    ...
    resolve(window: DateWindow, reference?: DateTime) {
        ...
        const ref = reference?.setZone(this.zone) || this.now();
        switch (window) {
            case "current day": {
    ...
    toDateTime(dateIsoOrMillis: Date | number | string) {
        if ("string" === typeof dateIsoOrMillis) {
            return DateTime.fromISO(dateIsoOrMillis, this.dateTimeOpts);
        } else if ("number" === typeof dateIsoOrMillis) {
     ...
     */
    static create(zone: Zone) {
        return freeze(new DateCalc(zone));
    }
}
